...
export class ScrollableBehavior extends Behavior {
  ...
  measure(node, constraints) {
    ...
    const canvasSize = node.context?.canvasManager?.getCanvasSize?.("main") ?? {
      width: 0,
      height: 600
    };

    const intrinsicWidth = Number.isFinite(maxWidth) ? maxWidth : canvasSize.width;
    ...
    const intrinsicHeight = Number.isFinite(maxHeight) ? maxHeight : canvasHeight;
